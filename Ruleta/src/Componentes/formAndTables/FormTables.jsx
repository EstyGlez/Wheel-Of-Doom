...
const AdminList = () => {
  ...
  useEffect(() => {
    getData();
    const savedDrawParticipants = JSON.parse(
      localStorage.getItem("drawParticipants") || "[]"
    );
  ...
  const handleDraw = () => {
    const winnerIndex = Math.floor(Math.random() * drawParticipants.length);
    const winnerId = drawParticipants[winnerIndex];
    const winner = adminList.find((user) => user.id === winnerId);
    if (winner) {
      alert(`¡El ganador del sorteo es: ${winner.userName} ${winner.surName}!`);
    } else {
  ...
  return (
    <section className="container">
      ...
      <section>
        <h1>Participantes para el sorteo:</h1>
      </section>
